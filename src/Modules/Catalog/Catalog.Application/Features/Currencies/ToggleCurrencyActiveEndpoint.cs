...
public static class ToggleCurrencyActiveEndpoint
{
        ...
        CancellationToken ct)
    {
        ...
        var client = mediator.CreateRequestClient<ToggleCurrencyActiveCommand>();

        var response = await client.GetResponse<Result>(
            new ToggleCurrencyActiveCommand(id, request.Active, actorAdminId), ct);
