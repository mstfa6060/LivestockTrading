...
public static class CreateBreedEndpoint
{
        ...
        CancellationToken ct)
    {
        ...
        var client = mediator.CreateRequestClient<CreateBreedCommand>();
        ...
        var response = await client.GetResponse<Result<int>, Result>(
            new CreateBreedCommand(dto, actorAdminId), ct);

        if (response.Is(out Response<Result<int>>? successResponse))
        {
            var result = successResponse!.Message;
            if (result.IsSuccess)
                return Results.Created($"/admin/catalog/breeds/{result.Value}", result.Value);
