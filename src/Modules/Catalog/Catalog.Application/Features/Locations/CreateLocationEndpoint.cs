...
public static class CreateLocationEndpoint
{
        ...
        CancellationToken ct)
    {
        ...
        var client = mediator.CreateRequestClient<CreateLocationCommand>();
        ...
        var response = await client.GetResponse<Result<int>, Result>(
            new CreateLocationCommand(dto, actorAdminId), ct);

        if (response.Is(out Response<Result<int>>? successResponse))
        {
            var result = successResponse!.Message;
            if (result.IsSuccess)
                return Results.Created($"/admin/catalog/locations/{result.Value}", result.Value);
