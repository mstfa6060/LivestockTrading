...
public sealed class UnlinkExternalLoginHandler : IConsumer<UnlinkExternalLoginCommand>
{
    private readonly IUserRepository _users;
    private readonly TimeProvider _clock;

    public UnlinkExternalLoginHandler(IUserRepository users, TimeProvider clock)
    {
        _users = users;
        _clock = clock;
    }

    public async Task Consume(ConsumeContext<UnlinkExternalLoginCommand> context)
    {
        ...
        try
        {
            ...
            user.UnlinkExternalLogin(context.Message.ExternalLoginId, now);
