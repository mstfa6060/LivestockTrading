...
public sealed class UpdatePushTokenHandler : IConsumer<UpdatePushTokenCommand>
{
    private readonly IUserRepository _users;
    private readonly TimeProvider _clock;

    public UpdatePushTokenHandler(IUserRepository users, TimeProvider clock)
    {
        _users = users;
        _clock = clock;
    }

    public async Task Consume(ConsumeContext<UpdatePushTokenCommand> context)
    {
        ...
        try
        {
            ...
            user.UpdateDevicePushToken(context.Message.DeviceId, context.Message.Dto.PushToken, now);
