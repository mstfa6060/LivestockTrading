...
public sealed class RevokeUserRoleHandler : IConsumer<RevokeUserRoleCommand>
{
    private readonly IUserRepository _users;
    private readonly TimeProvider _clock;

    public RevokeUserRoleHandler(IUserRepository users, TimeProvider clock)
    {
        _users = users;
        _clock = clock;
    }

    public async Task Consume(ConsumeContext<RevokeUserRoleCommand> context)
    {
        ...
        try
        {
            ...
            // Domain: aktif-role yoksa throw + UserRoleRevoked event (User.cs:374).
            user.RevokeRole(msg.Role, msg.ActorAdminId, now);
