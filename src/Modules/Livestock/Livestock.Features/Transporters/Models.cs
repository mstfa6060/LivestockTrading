...
public record TransporterListItem(Guid Id, Guid UserId, string CompanyName, TransporterStatus Status, double AverageRating, int ReviewCount, DateTime CreatedAt);
public record TransporterDetail(Guid Id, Guid UserId, string CompanyName, string? Description, string? PhoneNumber, string? Email, string? WebsiteUrl, string? LicenseNumber, string? LogoUrl, TransporterStatus Status, double AverageRating, int ReviewCount, DateTime? VerifiedAt, DateTime CreatedAt);
