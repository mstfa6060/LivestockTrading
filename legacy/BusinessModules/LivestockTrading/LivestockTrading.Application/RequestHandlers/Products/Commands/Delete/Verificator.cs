...
namespace LivestockTrading.Application.RequestHandlers.Products.Commands.Delete;
...
public class Verificator : IRequestVerificator
{
	...
	public async Task VerificateDomain(IRequestModel payload, EndpointContext context, CancellationToken cancellationToken)
	{
		...
		// Admin/Moderator can delete any product; Sellers can only delete their own
		if (!_permissionService.IsModerator())
		{
			var currentUserId = _currentUserService.GetCurrentUserId();
			await _dbVerification.ValidateProductOwnership(request.Id, currentUserId, cancellationToken);
		}
	}
}
