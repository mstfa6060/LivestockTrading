...
Write-Host "🔧 Using arfblocks-cli at: $arfExe" -ForegroundColor Green

function Invoke-ArfBlocks([string]$configPath) {
    if (-not (Test-Path $configPath)) { 
        Write-Host "⚠️  Config not found: $configPath" -ForegroundColor Yellow
        Write-Host "    Skipping this module..." -ForegroundColor Yellow
        return
    }
    Write-Host "▶️  Executing: $configPath" -ForegroundColor Cyan
...
function Ensure-GitRepo([string]$path) {
    if (-not (Test-Path $path)) { 
        Write-Host "⚠️  Folder not found: $path" -ForegroundColor Yellow
        return $false
    }
    if (-not (Test-Path (Join-Path $path ".git"))) { 
        Write-Host "⚠️  Not a git repo: $path" -ForegroundColor Yellow
        return $false
    }
    return $true
}
...
Write-Host ""
Write-Host "📦 ArfBlocks CLI output is being generated..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "✅ CLI output completed." -ForegroundColor Green
Pop-Location

# === 2. Error Code Export ===
Write-Host ""
Write-Host "🔄 Running Error Code Export..." -ForegroundColor Yellow
...
if (Test-Path $exporterPath) {
    ...
    Write-Host "⚠️  Make sure ErrorCodeExporter includes LivestockTradingDomainErrors!" -ForegroundColor Yellow
    
    dotnet run
    $code = $LASTEXITCODE
    if ($code -ne 0) { 
        Write-Host "⚠️  ErrorCodeExporter failed. Code: $code" -ForegroundColor Yellow
        Write-Host "    Continuing anyway..." -ForegroundColor Yellow
    } else {
        Write-Host "✅ Error Code Export completed." -ForegroundColor Green
    }
    Pop-Location
}
else {
    Write-Host "⚠️  ErrorCodeExporter path not found: $exporterPath" -ForegroundColor Yellow
...
if (Ensure-GitRepo $webPath) {
    Write-Host ""
    Write-Host "🚀 Pushing to Web..." -ForegroundColor Yellow
    Push-Location $webPath
    
    git pull --rebase --autostash
    if ($LASTEXITCODE -ne 0) { 
        Write-Host "⚠️  git pull (web) failed, continuing..." -ForegroundColor Yellow
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Web push completed." -ForegroundColor Green
    } else {
        Write-Host "⚠️  Web push failed." -ForegroundColor Yellow
    }
    
    Pop-Location
}
else {
    Write-Host "⚠️  Web path not found or not a git repo: $webPath" -ForegroundColor Yellow
...
if (Ensure-GitRepo $mobilePath) {
    Write-Host ""
    Write-Host "📱 Pushing to Mobile..." -ForegroundColor Yellow
    Push-Location $mobilePath
    
    git pull --rebase --autostash
    if ($LASTEXITCODE -ne 0) { 
        Write-Host "⚠️  git pull (mobile) failed, continuing..." -ForegroundColor Yellow
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Mobile push completed." -ForegroundColor Green
    } else {
        Write-Host "⚠️  Mobile push failed." -ForegroundColor Yellow
    }
    
    Pop-Location
}
else {
    Write-Host "⚠️  Mobile path not found or not a git repo: $mobilePath" -ForegroundColor Yellow
