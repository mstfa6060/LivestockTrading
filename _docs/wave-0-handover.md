...
**Durum:** Wave 0 KAPANDI, rebuild/v2 = dd50923, tag wave-0-complete
...
| Branch | SHA | Durum |
|---|---|---|
| `main` | `44416138...` | ArfBlocks production, Wave 0 boyunca DOKUNULMADI |
| `rebuild/v2` | `dd50923...` | Wave 0 içeriyor, FF merge ile geldi |
| `feature/wave-0-infra` | `dd50923...` | Wave 0 origin branch'i, referans olarak korunuyor |
| tag `wave-0-complete` | → `dd50923` | Wave 0 bitiş sınırı (annotated) |

## Wave 0 Commit Zinciri (10 commit + kapanış)
...
7750d26 feat(infra): Phase 5 - docker compose (pg+redis+rabbit+minio)
...
4441613 fix(signalr): ArfBlocks parent (production main, dokunulmadi)
...
| `WAVE-0-CLEANUP-01` | 7 yabancı `claude/*` branch (eski ArfBlocks "entity endpoints") — `git branch -D` lokal temizlik | Düşük, Wave 1+ |
| `WAVE-0-CLEANUP-02` | `.gitattributes` yok — markdown LF normalize için eklenmeli | Düşük |
| `WAVE-0-CLEANUP-03` | `.gitignore` dead refs (eski ArfBlocks path'leri) — temizlik | Düşük |
...
4. **Koşullu talimat yok:** Bir önceki adımın çıktısı review edilmeden sonraki adımın talimatı üretilmez (Sapma 10 dersi)
5. **Prompt'a değil fiili veriye güven:** Hash/SHA/sayım vb. değerler her zaman fiili kaynaktan teyit edilir (Sapma 17, 19 dersleri)
