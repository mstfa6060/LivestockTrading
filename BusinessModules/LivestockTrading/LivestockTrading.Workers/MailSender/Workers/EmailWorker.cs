...
namespace LivestockTrading.Workers.MailSender.Workers;
...
public class EmailWorker : BackgroundService
{
    ...
    private const string ExchangeName = "livestocktrading.notification.email";
    private const string QueueName = "livestocktrading.notification.email.queue";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            _logger.LogError(ex, "EmailWorker encountered an error");
        }
    }
    ...
    private async Task<bool> TryHandleEvents(string message, IServiceScope scope)
    {
        ...
        try
        {
            ...
            {
                _logger.LogInformation("Handling StudentCreatedEvent for student: {StudentId}", studentCreatedEvent.StudentId);
                var handler = scope.ServiceProvider.GetRequiredService<StudentCreatedEmailHandler>();
        ...
        try
        {
            ...
            {
                _logger.LogInformation("Handling StudentUpdatedEvent for student: {StudentId}", studentUpdatedEvent.StudentId);
                var handler = scope.ServiceProvider.GetRequiredService<StudentUpdatedEmailHandler>();
        ...
        try
        {
            ...
            {
                _logger.LogInformation("Handling StudentDeletedEvent for student: {StudentId}", studentDeletedEvent.StudentId);
                var handler = scope.ServiceProvider.GetRequiredService<StudentDeletedEmailHandler>();
