...
///
/// NOT: Conversation membership server-side check'i şu an REST tarafında
/// (Verificator/DbVerificationService) yapılıyor. Hub'da DbContext bağımlılığı
/// olmadığı için Join çağrılarında client güvenine bel bağlanır — kullanıcı
/// kendi conversation'larından başkasına join etmemeli. Sızıntı riski:
/// kötü niyetli bir kullanıcı başka conversation'a join edip yayınlanan
/// mesajları görebilir. Bunu kapatan task: MST-187 (sub-task) — Hub'da
/// IServiceScopeFactory ile membership re-check planlanıyor.
/// </summary>
[Authorize]
public class ChatHub : Hub
{
	...
	public async Task<List<OnlineUserInfo>> GetOnlineUsers(List<Guid> userIds)
	{
		...
		foreach (var userId in userIds)
		{
			...
			result.Add(new OnlineUserInfo { UserId = userId, IsOnline = isOnline });
		}
		return result;
	}
