...
public class Validator : IRequestValidator
{
	...
	{
		var request = (RequestModel)payload;
		await _dbValidator.ValidateProductExists(request.ProductId, cancellationToken);
	}
}
...
public class RequestModel_Validator : AbstractValidator<RequestModel>
{
	public RequestModel_Validator()
	{
		...
		RuleFor(x => x.DocumentUrl)
			.NotEmpty()
			.WithMessage(ErrorCodeGenerator.GetErrorCode(() => LivestockTradingDomainErrors.ProductDocumentErrors.DocumentUrlRequired));

		RuleFor(x => x.FileName)
			.NotEmpty()
			.WithMessage(ErrorCodeGenerator.GetErrorCode(() => LivestockTradingDomainErrors.ProductDocumentErrors.DocumentFileNameRequired));
	}
}
